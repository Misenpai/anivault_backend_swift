...
final class AnimeService {
    ...
    func updateAnimeStatus(
        ...
        guard let animeStatus = try await animeRepository.findByUserAndMalId(
            userEmail: userEmail,
            malId: malId,
            on: db
        ) else {
    ...
    func getUserAnimeStats(userEmail: String, on db: Database) async throws -> AnimeStatsDTO {
        ...
        let totalEpisodes = try await animeRepository.totalEpisodesWatched(userEmail: userEmail, on: db)

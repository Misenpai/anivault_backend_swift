...
struct Constants {
    ...
    struct JWT {
        ...
        static let expiration: TimeInterval = TimeInterval(Environment.get("JWT_EXPIRATION").flatMap(Int.init) ?? 604800) 
        static let refreshExpiration: TimeInterval = TimeInterval(Environment.get("JWT_REFRESH_EXPIRATION").flatMap(Int.init) ?? 2592000) 
    ...
    struct User {
        struct Username {
            ...
            static let reservedNames = ["admin", "root", "system", "api", "www", "mail", "support", "help", "info"]
        }
    ...
    struct Security {
        ...
        struct CORS {
            static let allowedOrigins = Environment.get("ALLOWED_ORIGINS")?.split(separator: ",").map(String.init) ?? ["*"]
            static let allowedMethods = ["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"]
            static let allowedHeaders = ["Accept", "Authorization", "Content-Type", "Origin", "X-Requested-With"]
        }
    }
    ...
    struct Features {
        static let emailVerificationRequired = Environment.get("EMAIL_VERIFICATION_REQUIRED") == "true"
        ...
        static let animeRecommendationsEnabled = Environment.get("ANIME_RECOMMENDATIONS_ENABLED") == "true"
...
extension String {
    ...
    func isValidEmail() -> Bool {
        let emailPredicate = NSPredicate(format: "SELF MATCHES[c] %@", Constants.Validation.emailRegex)
        return emailPredicate.evaluate(with: self)
    }
    
    
    func isValidUsername() -> Bool {
        let usernamePredicate = NSPredicate(format: "SELF MATCHES %@", Constants.Validation.usernameRegex)
